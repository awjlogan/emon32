...
main(int argc, char *argv[])
{
    ...
    (void)dataPackage(&dataset, txBuffer, 1024);
    (void)snprintf(goldBuffer, 1024, "MSG:%d,Vrms:%d,P1:%d,E1:%d,P2:%d,E2:%d,P3:%d,E3:%d,P4:%d,E4:%d",
    ...
    if (0 != strcmp(txBuffer, goldBuffer))
    {
        printf("  all data == 0:\n");
        printf("    %s\n", txBuffer);
        assert(0);
